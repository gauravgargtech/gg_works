...
function startScheduler() {
  ...
  console.log(`[scheduler] technical-only: "${technicalCron}" | full pipeline: "${fullCron}"`);
  ...
  cron.schedule(fullCron, () => {
    runFullPipeline().catch((err) => console.error("[scheduler] full pipeline run failed:", err));
  });
}
