...
const calculateObLuxalgo = async (candles) => {
...
`;
  ...
  const result = await pineTS.run(OB_SCRIPT);

  const obSeries = result.plots["__lines__"].data?.[0]?.value;

  const response = [];
  const latestCandle = candles[candles.length - 1];
  for (const data of obSeries) {
    if (!data["_deleted"]) {
      response.push({
        value: data["y1"],
        theTime: dayjs(data["x1"]).tz("Australia/Brisbane").unix(),
        theTimeF: dayjs(data["x1"])
          .tz("Australia/Brisbane")
          .format("YYYY-MM-DD HH:mm:ss"),
        is_bull: data["y1"] > latestCandle.close ? true : false,
      });
    }
  }
...
module.exports = calculateObLuxalgo;
