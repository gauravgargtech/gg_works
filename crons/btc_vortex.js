...
async function checkVortexBTC() {
  ...
  if (thirdLastTSI < thirdLastSignal && currentTSI > currentSignal) {
    ...
    await sendPushNotif(
      `BTC Vortex Crossover 1 Hour Direction changed to BULLISH at ${currentPrice}`,
    );
    ...
    await sendPushNotif(
      `BTC Vortex Crossover 1 Hour Direction changed to BEARISH at ${currentPrice}`,
    );
    await del(`tsi_${symbol}_direction_1hrs_ups`);
    await del(`tsi_${symbol}_direction_1hrs_downs`);
  }
  ...
  if (
    ...
    if (!isCC) {
      await sendPushNotif(
        `BTC Vortex Crossover 1 Hour : ${symbol} - BULLISH at ${currentPrice}`,
      );
    ...
    if (!isCC) {
      await sendPushNotif(
        `BTC Vortex Crossover 1 Hour : ${symbol} - BEARISH at ${currentPrice}`,
      );
