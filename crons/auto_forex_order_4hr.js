...
// ─── Main ─────────────────────────────────────────────────────
async function autoForexOrder4Hr() {
  ...
  for (const symbol of FOREX_PAIRS) {
    ...
    const candles = await getCandles(symbol.replace("_", ""), "4h", 800);
    ...
    if (
      ...
      if (placeNew) {
        await sendPushNotif(
          `${symbol} at 4 Hour - Placing Order, BULLISH,  at ${closes[closes.length - 1]}`,
        );
      }
      ...
      if (placeNew) {
        ...
        await sendPushNotif(
          `${symbol} at 4 Hour - Placing Order, BEARISH,  at ${closes[closes.length - 1]}`,
        );
      }
...
module.exports = autoForexOrder4Hr;
