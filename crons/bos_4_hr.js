...
const runner = async (coin = "BTCUSDT", theTimeFrame = "60") => {
  const pineTS = new PineTS(Provider.Binance, coin, theTimeFrame, 500);

  const all = await pineTS.run(`
//@version=5
...
`);
  ...
  if (all?.plots?.["__boxes__"]?.data?.[0]?.value) {
    return all.plots["__boxes__"].data[0].value;
  }
...
const calculateResistanceSupport = async (theTimeFrame = "240") => {
  for (const coin of CRYPTO_PAIRS) {
    const dataFromPine = await runner(coin, theTimeFrame);
    await sleep(2000);
...
module.exports = calculateResistanceSupport;
